...
def infercnv(
    ...
    *,
    ...
    """(From CanSig, https://github.com/BoevaLab/CanSig/tree/main/src/cansig)
...
def set_cnv(adata, chr_pos, X_cnv, STEP, WINDOW_SIZE):
    '''
    Updates chromosome positions, infercnv arguments and CNV matrix after inferring the CNVs.
    
    Args: 
        adata: Anndata object (annotated data matrix)
        chr_pos: Dictionary mapping each chromosome to the index this chromosome begins on X_cnv
        X_cnv: Sparse matrix containing inferred CNV values
        STEP: step size used for inferCNV
        WINDOW_SIZE: window size used for inferCNV
    '''
    adata.uns = {'cnv': {'chr_pos': chr_pos , 'step': STEP,'window_size': WINDOW_SIZE}}
    adata.obsm['X_cnv'] = X_cnv
   
    
    
def set_subclonal_cnv(adata, cnv_key = 'X_cnv', subclonal_key = 'subclonal'):
    '''
    Homogeneizes the CNV on a subclonal level rather than on a cell level and updates X_cnv of anndata object to 
    cnv array discretized and homogeneized to a subclone level.

    Args:
        adata: anndata object with precomputed cluster labels and CNV calls

    '''
    result = cansig.cnvanalysis.differentialcnvs.get_subclonal_cnv(adata, cnv_key = cnv_key, subclonal_key= subclonal_key)

    #to make sure order matches
    result = result.loc[adata.obs.iloc[:,0].index]
    sparse_result = scipy.sparse.csr_matrix(result.values)
    adata.obsm['X_cnv'] = sparse_result



def get_cnv_mapping(data):
    '''
    Computes the mapping between the CNV regions and the genes used for the computation of the CNV call.
    Modified version of CanSig's get_cnv_mapping()
    (https://github.com/BoevaLab/CanSig/blob/main/src/cansig/cnvanalysis/differentialcnvs.py)
    
    Args: 
         data: anndata object with precomputed cluster labels and CNV calls
         
    Returns: 
        Dataframe containing chromosome number, start and end location on chromosome for each column of X_cnv (i.e.
        for each CNV segment)
    '''
    ...
    for i, chrom in enumerate(chromosomes):
        ...
        for i in range(len(mapping) - 1):
            ...
            start.append(region_start)
            end.append(region_end)
    
    
    zipped = list(zip(chr_list, start, end))

    df = pd.DataFrame(zipped, columns=['chromosome', 'begin', 'end'])    
    df['size'] = df['end']- df['begin'] + 1
    return df
    
    
def get_num_genes(adata, x):
    '''
    Computes number of genes in each CNV segment including filtered genes (i.e. all genes whose genomic position falls within 
    the boundaries of the chromosome location of the CNV segment). 
    
    Args: 
         data: anndata object with precomputed cluster labels and CNV calls
         x: CNV segment with 'chromosome','begin' and 'end' annotation  (one row of cnv_mapping)
         
    Returns: 
        Number of genes falling within that CNV segment
    '''
    
    chr_genes = adata.var[adata.var['chromosome'] == x['chromosome']]
    genes = chr_genes[chr_genes['start'] >= x['begin']]
    genes = genes[genes['end'] <= x['end']]
    return len(genes)

        
def get_cnv_called(adata, excluded_chromosomes = ("chrX", "chrY",'chrM'), count_threshold = 0.1):
    '''
    Returns a boolean vector indicating if a gene has been used to call CNVs.
    
    Args: 
        adata: anndata object
        excluded_chromosome: tuple of chromosomes excluded in inferCNV (default = ("chrX", "chrY",'chrM'))
        count_threshold: threshold for filtering genes (default = 0.1) 
    '''
    mean_counts_per_gene = np.asarray(adata.X.mean(0)).ravel()
    var = adata.var
    cnv_called = (var['chromosome'].notnull().values
                  & ~var['chromosome'].isin(excluded_chromosomes).values
                  & (mean_counts_per_gene >= count_threshold))
